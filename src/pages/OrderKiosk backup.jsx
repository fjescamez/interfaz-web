...
import { handleExceptions, kioskConfigAuto, resetKiosk } from "../components/orderKioskComponents/configBehavior";
...
function OrderKiosk({ configMode }) {
  ...
  const [countOtraDoc, setCountOtraDoc] = useState({});
    ...
    if (chosenKiosk !== "Automática" && chosenKiosk !== "Manual") {
      ...
      if (orderXml && actividad) {
        if (chosenKiosk === "Automática") {
          ...
          resetKiosk({ setIsActive, setIsOpen });
        }
      }
    }

    if (orderXml?.tecnicos && orderXml?.tecnicos?.trapping !== " ") {
      const trapping = orderXml?.tecnicos?.trapping.split(" ")[0];
  ...
  useEffect(() => {
    if (Object.keys(otraDocumentacion).length > 0) {
      ...
      const certificadosTrue = tiposCertificado.filter((key) => otraDocumentacion[key]).length;
      const etiquetasTrue = tiposEtiqueta.filter((key) => otraDocumentacion[key]).length;
  ...
  const components = {
    ...
    "otraDocumentacion": {
      title: `Certificados: ${countOtraDoc.certificados || 0} | Etiquetas: ${countOtraDoc.etiquetas || 0} | Separaciones: ${otraDocumentacion.separaciones?.hacer ? 'Sí' : 'No'}`,
      component: <OtraDocComponent otraDocumentacion={otraDocumentacion} setOtraDocumentacion={setOtraDocumentacion} setCountOtraDoc={setCountOtraDoc} />,
  ...
  const navigateToConfig = () => {
    ...
    createTab(path, `CONFIG. KIOSCO ${cliente?.name || ""}`);
  }
